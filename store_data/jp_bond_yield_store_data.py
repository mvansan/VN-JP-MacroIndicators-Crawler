...
def get_s3_key(date_str):
    ...
    if date <= cutoff_date:
        return "uploads/jp_bond_yield/history.csv"
    else:
        return f"uploads/jp_bond_yield/{date.strftime('%d-%m-%Y')}.csv"

def extract_date_from_key(key):
    m = re.search(r'jp_bond_yield/(\d{2}-\d{2}-\d{4})\\.csv', key)
    if not m:
        m = re.search(r'jp_bond_yield/(\d{2}-\d{2}-\d{4})\.csv', key)
    if m:
...
def process_and_upload_data():
    try:
        ...
        data_path = "data/cleaned/jp_bond_yield_cleaned.csv"
        ...
        print("All JP bond yield data has been processed and uploaded successfully!")
